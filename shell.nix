...
let
  fhs =  pkgs.buildFHSEnv rec{
    ...
    profile = ''
      ...
      # Install packages only if UFold.yaml has been updated
      if [ ! -f $MAMBA_ROOT_PREFIX/envs/${name}/.env-hash ] || ! sha256sum -c $MAMBA_ROOT_PREFIX/envs/${name}/.env-hash; then
        echo "Installing/updating packages from UFold.yaml..."
        micromamba install -y -n ${name} -f UFold.yaml -c conda-forge
        sha256sum UFold.yaml > $MAMBA_ROOT_PREFIX/envs/${name}/.env-hash
      else
